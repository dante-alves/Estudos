...
void escrever(char f[]){
	...
	char strDia[3], strMes[3];
	int opc;
	
	if(file){
		do{
			printf("Insira o nome do aniversariante: ");
			scanf("%50[^\n]", aniversariante.nome);
			...
			//Botar o zero no dia ou mês
			if(aniversariante.dia < 10)
				snprintf(strDia, 3,"0%d", aniversariante.dia);
			
			
			if(aniversariante.mes < 10)
				snprintf(strMes, 3,"0%d", aniversariante.mes);
